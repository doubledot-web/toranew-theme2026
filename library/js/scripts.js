...
jQuery(document).ready(function($) {
			...
			$('.counter span').each(function() {
				...
				$({
					...
					{ countNum: countTo },
					{
						...
						easing: 'swing',
						step: function() {
							$this.text(Math.floor(this.countNum).toLocaleString('el'));
						},
						complete: function() {
	...
	function locationsCountSelectorPosition() {
		$('.locations-count-selector').css('left', $(".results-indicator").width() + 'px');
	}
	...
	function paginationInit(pageSize) {
		...
		var paginationArgs = {
			...
			afterPaging: function(activePage) {
				...
				$('.results-indicator').html('<span>Αποτελέσματα</span><span>' + fromResults + ' - ' + toResults + ' από ' + totalLocations + '</span>');
